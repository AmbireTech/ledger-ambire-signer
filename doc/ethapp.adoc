...
A field can carry a `PARAM_GROUP` parameter (type `0x0c`) to logically group multiple
sub-fields under a single descriptor. Each sub-field is a fully encoded `FIELD` struct
embedded in the group payload. The group supports two iteration strategies for array
fields: `SEQUENTIAL` (all elements of each sub-field before moving to the next) and
`BUNDLED` (sub-fields interleaved by array index — not yet fully implemented, falls back
to `SEQUENTIAL`).

See link:tlv_structs.md#param_group[PARAM_GROUP struct] for the full field layout.

Each field can carry a `PARAM_TRUSTED_NAME` parameter, which supports two value types:

* *STANDARD* (default): the value is a 20-byte EVM address.
* *INTEROPERABLE* (EIP-7930): the value is a variable-length binary blob encoding
  `[chain_id (1–8 bytes, big-endian)][address (20 bytes)]`.
  The device extracts the chain ID and EVM address, may display the chain as a
  network-name suffix, and applies trusted-name resolution to the address part.

See link:tlv_structs.md#param_trusted_name[PARAM_TRUSTED_NAME struct] for the full field layout.
...
The TLV description is available link:tlv_structs.md#network_info[here]
...
The TLV description is available link:tlv_structs.md#tx_simulation[here]
...
| *Description*                                                   | *Length (byte)*
| Payload length                                                  | 2
| link:tlv_structs.md#gating_descriptor[GATING_DESCRIPTOR struct] | variable
|====================================================================
...
This command provides the app with a key/value entry for a specific contract, function selector,
and chain ID.  The entry is signed by CAL and stored on the device until a new signing session
starts.  When the Generic Transaction Parser encounters a `MAP_REF` parameter, the device looks
up the stored entries and resolves the display value from the matching `MAP_ENTRY`.
...
| *Description*                                       | *Length (byte)*
| Payload length                                      | 2
| link:tlv_structs.md#map_entry[MAP_ENTRY struct]     | variable
|======================================================================
