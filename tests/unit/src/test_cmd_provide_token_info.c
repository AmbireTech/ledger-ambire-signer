...
tlv_dynamic_descriptor_status_t __wrap_tlv_use_case_dynamic_descriptor(
...
static void test_unknown_p1_rejected(void **state) {
    (void) state;
    uint8_t apdu[100];
...
static void test_legacy_p2_nonzero_rejected(void **state) {
    (void) state;
    uint8_t apdu[100];
    s_legacy_opts opts = default_legacy();
    size_t len = build_legacy(apdu, sizeof(apdu), &opts);
...
static void test_legacy_happy_path(void **state) {
    (void) state;
    uint8_t apdu[200];
    s_legacy_opts opts = default_legacy();
    size_t len = build_legacy(apdu, sizeof(apdu), &opts);
    ...
    assert_non_null(got);
    assert_memory_equal(got->ticker, "USDC", 4);
    assert_int_equal(got->decimals, 6);
    assert_int_equal(got->chain_id, 1);
}

static void test_legacy_truncated_missing_ticker(void **state) {
    (void) state;
    uint8_t apdu[100];
    s_legacy_opts opts = default_legacy();
...
static void test_legacy_truncated_missing_address(void **state) {
    (void) state;
    uint8_t apdu[100];
    s_legacy_opts opts = default_legacy();
    opts.truncate_at = 1;
...
static void test_legacy_truncated_missing_decimals(void **state) {
    (void) state;
    uint8_t apdu[100];
    s_legacy_opts opts = default_legacy();
    opts.truncate_at = 2;
...
static void test_legacy_truncated_missing_chain_id(void **state) {
    (void) state;
    uint8_t apdu[100];
    s_legacy_opts opts = default_legacy();
    opts.truncate_at = 3;
...
static void test_legacy_missing_signature_rejected(void **state) {
    (void) state;
    uint8_t apdu[100];
    s_legacy_opts opts = default_legacy();
...
static void test_legacy_decimals_above_uint8_rejected(void **state) {
    (void) state;
    uint8_t apdu[200];
    s_legacy_opts opts = default_legacy();
...
static void test_legacy_unsupported_chain_id_rejected(void **state) {
    (void) state;
...
static void test_legacy_signature_failure_does_not_store(void **state) {
    (void) state;
    ...
    memset(addr, 0xAA, sizeof(addr));
    uint64_t chain = 1;
    assert_null(get_matching_token_info(&chain, addr));
}

static void test_legacy_ticker_too_long_rejected(void **state) {
    (void) state;
...
static void test_legacy_duplicate_returns_same_index(void **state) {
    (void) state;
    uint8_t apdu[200];
    s_legacy_opts opts = default_legacy();
    size_t len = build_legacy(apdu, sizeof(apdu), &opts);
...
static void test_legacy_second_descriptor_gets_next_index(void **state) {
    (void) state;
    uint8_t apdu[200];
...
static void test_tlv_use_case_failure_rejects(void **state) {
    (void) state;
...
static void test_tlv_coin_type_mismatch_rejects(void **state) {
    (void) state;
...
static void test_tlv_happy_path_stores(void **state) {
    (void) state;
    uint8_t tuid[32];
    size_t tuid_len = build_tuid(tuid, sizeof(tuid), 1, 0xAA);
    g_tlv_uc_tuid_ptr = tuid;
...
static void test_tlv_missing_tuid_tags_rejected(void **state) {
    (void) state;
...
int main(void) {
    const struct CMUnitTest tests[] = {
        cmocka_unit_test_setup(test_unknown_p1_rejected, reset),
        cmocka_unit_test_setup(test_legacy_p2_nonzero_rejected, reset),
        cmocka_unit_test_setup(test_legacy_happy_path, reset),
        cmocka_unit_test_setup(test_legacy_truncated_missing_ticker, reset),
        cmocka_unit_test_setup(test_legacy_truncated_missing_address, reset),
        cmocka_unit_test_setup(test_legacy_truncated_missing_decimals, reset),
        cmocka_unit_test_setup(test_legacy_truncated_missing_chain_id, reset),
        cmocka_unit_test_setup(test_legacy_missing_signature_rejected, reset),
        cmocka_unit_test_setup(test_legacy_decimals_above_uint8_rejected, reset),
        cmocka_unit_test_setup(test_legacy_unsupported_chain_id_rejected, reset),
        cmocka_unit_test_setup(test_legacy_signature_failure_does_not_store, reset),
        cmocka_unit_test_setup(test_legacy_ticker_too_long_rejected, reset),
        cmocka_unit_test_setup(test_legacy_duplicate_returns_same_index, reset),
        cmocka_unit_test_setup(test_legacy_second_descriptor_gets_next_index, reset),
        cmocka_unit_test_setup(test_tlv_use_case_failure_rejects, reset),
        cmocka_unit_test_setup(test_tlv_coin_type_mismatch_rejects, reset),
        cmocka_unit_test_setup(test_tlv_happy_path_stores, reset),
        cmocka_unit_test_setup(test_tlv_missing_tuid_tags_rejected, reset),
    };
